...
int __mfc_rm_check_load_balancing_condition(struct mfc_ctx *ctx, int load_add)
{
	...
	if (dev->move_ctx_cnt || (load_add == MFC_RM_LOAD_DELETE) ||
			...
			&& !(IS_SWITCH_SINGLE_MODE(ctx) && !ctx->has_last_frame))) {
		...
		struct mfc_ctx *ctx)
{
	...
	if (core->sched->enqueue_work(core, core_ctx)) {
		if (IS_SWITCH_SINGLE_MODE(ctx) && dev->num_inst == 1 && !ctx->has_last_frame) {
			mfc_ctx_debug(2, "[RM] try to change single to multi mode\n");
