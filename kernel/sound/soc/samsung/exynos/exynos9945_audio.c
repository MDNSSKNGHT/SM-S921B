...
/*
 *  Sound Card Driver for Exynos9945
 *
...
static int parse_component_dapm_routing(struct snd_soc_card *card,
			struct snd_soc_component *component, const char *propname)
{
	...
	dev_info(card->dev, "%s: component %s\n", __func__, component->name);

	num_routes = of_property_count_strings(np, propname);
	if (num_routes < 0 || num_routes & 1) {
		dev_err(card->dev,
			"%s: invalid prop '%s' num_routes %d\n", __func__, propname, num_routes);
		return -EINVAL;
	}
	num_routes /= 2;
	...
	routes = devm_kcalloc(card->dev, num_routes, sizeof(*routes),
			      GFP_KERNEL);
	if (!routes) {
		dev_err(card->dev, "%s: failed to alloc DAPM route\n", __func__);
		return -ENOMEM;
	}
	...
	for (i = 0; i < num_routes; i++) {
		...
		if (ret) {
			dev_err(card->dev,
				"%s: prop '%s' failed to read %d-th ret %d\n",
	...
	snd_soc_dapm_add_routes(&card->dapm, routes, num_routes);
...
static int uaif3_init(struct snd_soc_pcm_runtime *rtd)
{
...
#if IS_ENABLED(CONFIG_SND_SOC_CS35L43)
	if (strstr(component->name, "cs35l43")) {
		dev_info(card->dev, "%s: add dapm route for %s\n", __func__, component->name);
		parse_component_dapm_routing(card, component, "cs35l43,routing");
					...
					struct snd_pcm_hw_params *params)
{
	...
	int ret;
#if IS_ENABLED(CONFIG_SND_SOC_CS35L43) || IS_ENABLED(CONFIG_SND_SOC_CS40L26)
	...
	case ABOX_UAIF_DAI_ID(0, 0):
		...
		for_each_rtd_codec_dais(rtd, i, codec_dai) {
			ret = snd_soc_component_set_sysclk(codec_dai->component,
						...
						SND_SOC_CLOCK_IN);
			if (ret < 0) {
				dev_err(card->dev, "%s: set haptic sysclk failed: %d\n",
					codec_dai->name, ret);
				return ret;
			}
		}
	...
	case ABOX_UAIF_DAI_ID(0, 3):
		...
		if (ret < 0) {
			dev_err(card->dev, "set tristate failed: %d\n", ret);
			return ret;
		}
...
static const struct snd_soc_dapm_widget exynos_widgets[] = {
	...
	SND_SOC_DAPM_SPK("HAPTIC SPK", NULL),
...
MODULE_DESCRIPTION("ALSA SoC Exynos9945 Sound Driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:exynos9945-audio");
